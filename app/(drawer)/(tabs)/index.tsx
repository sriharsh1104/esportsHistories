...
export default function NewsScreen() {
  ...
  const [activeCategory, setActiveCategory] = useState<"mobile" | "pc">(
    "mobile",
  );
    ...
    return confirmedSelectedGames
      .map((g: any) => {
        ...
        const fromCatalog =
          ...
          availableGames.find(
            (ag) => ag.name.toLowerCase() === name.toLowerCase(),
          );
  ...
  const hasFollowFeed = feedChips.length > 0;
  ...
  const renderGameSelectionCard = (modalLike: boolean) => (
    <Card
      style={{
        padding: w(16),
        marginBottom: h(16),
        borderColor: modalLike ? colors.tint : colors.border,
        borderWidth: 1,
      }}
    >
      ...
      <Text
        style={[
          styles.cardDesc,
          { color: colors.tabIconDefault, marginBottom: h(12) },
        ]}
      >
      ...
      <View style={{ flexDirection: "row", flexWrap: "wrap" }}>
        ...
        {!isGamesLoading && categoryGames.length === 0 && (
          <View style={{ width: "100%", marginBottom: h(8) }}>
            <ClassicEmptyState
              title="No game options"
              message="We couldn't load games for this category. Try again later."
              icon="gamepad"
          ...
          {isAuthenticated &&
            ...
            gamesFromProfile.length > 0 && (
              <View style={styles.newsSection}>
                ...
                {isDashboardLoading ? (
                  <View
                    style={{ paddingVertical: h(24), alignItems: "center" }}
                  >
                    <ActivityIndicator size="small" color={colors.tint} />
                  </View>
                  ...
                  dashboardItems.map((item) => (
                    <Card key={item.id} style={{ marginBottom: h(12) }}>
                      ...
                      >
                        <Text style={{ color: colors.accent, fontSize: w(12) }}>
                          {item.game ?? ""}
                        </Text>
                        <Text
                          style={{
